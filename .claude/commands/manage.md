...
**`fee_compounding`:**
- In range, unclaimed fees > $5 → `node cli.js claim --position <addr>` then `node cli.js add-liquidity --position <addr> --pool <pool> --amount-y <claimed_sol>` to re-add fees back
- OOR → close normally
