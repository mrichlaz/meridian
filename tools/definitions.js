...
Screening: minFeeActiveTvlRatio, minTvl, maxTvl, minVolume, minOrganic, minQuoteOrganic, minHolders, minMcap, maxMcap, minBinStep, maxBinStep, timeframe, category, minTokenFeesSol, excludeHighSupplyConcentration, allowedLaunchpads, blockedLaunchpads, screeningSource, useDiscordSignals, discordSignalMode, avoidPvpSymbols, blockPvpSymbols, maxBundlePct, maxBotHoldersPct, maxTop10Pct, minTokenAgeHours, maxTokenAgeHours, athFilterPct, maxEntryPriceRunup1hPct, maxEntryPriceDrop1hPct
Management: minClaimAmount, outOfRangeBinsToClose, outOfRangeWaitMinutes, belowRangeExitMinutes, maxHoldMinutes, conversionExitPct, conversionExitPnlPct, oorCooldownTriggerCount, oorCooldownHours, repeatDeployCooldownEnabled, repeatDeployCooldownTriggerCount, repeatDeployCooldownHours, repeatDeployCooldownScope, repeatDeployCooldownMinFeeEarnedPct, maxDeploysPerToken24h (max deploys into the same token per rolling 24h, 0=off), minVolumeToRebalance, stopLossPct, stopLossConfirmMinutes (minutes a shallow stop breach must hold before closing, 0=off; breaches >4pp past the stop close immediately), takeProfitPct, minSolToOpen, deployAmountSol, gasReserve, positionSizePct, minFeePerTvl24h, minAgeBeforeYieldCheck, trailingTakeProfit, trailingTriggerPct, trailingDropPct, trailingRetracePct, pnlSanityMaxDiffPct, solMode, autoSwapAfterClaim, evolveEnabled, thresholdEvolveEnabled
  (trailing TP is a ratchet: exits when PnL drops from peak by max(trailingDropPct, peak × trailingRetracePct). belowRangeExitMinutes closes a position whose whole range is above the price — fully converted inventory. maxHoldMinutes closes positions past that age unless trailing TP is active, 0 disables. conversionExitPct closes an in-range position once that % of the ladder has converted to base token with PnL at or below conversionExitPnlPct (negative number, e.g. -5; 0 disables the PnL floor so the rule fires on any negative PnL); set conversionExitPct=0 to disable the conversion exit rule entirely.)
Policy engine (deterministic screening gate): policyEnabled, policyMinFeeVolatilityRatio (hard EV floor: fee/volatility), policyMinVolumePersistence, policyToxicFlowPenalty, policyNeutralMinScore, policyRiskOffMinScore, policyRiskOnMinScore, policyShrinkRetryPct, policyQuietHoursUtc (UTC hour ranges where deploys size down, e.g. "8-12,20-24", empty=off), policyQuietHoursSizeMult (deploy size multiplier inside quiet hours, 0.1-1), policyQuietHoursAuto (re-learn quiet hours from close history every evolve tick; false pins manual windows)
