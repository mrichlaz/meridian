...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    if (!posAccount || posAccount.owner.equals(SystemProgram.programId)) {
      ...
      const externalCloseReason = reason || tracked?.close_reason || "external close via wallet";

      // Backfill pool-memory + lessons so external closes count toward the
      // repeat-deploy cooldown. Without this, a position closed in Phantom /
      // Solfare resets the streak and the agent can keep redeploying into the
      ...
      if (initialUsd > 0 && tracked) {
        try {
          ...
          await recordPerformance({
            ...
            base_mint: poolMeta?.base_mint || tracked.base_mint || null,
            strategy: tracked.strategy || "spot",
            bin_range: tracked.bin_range || null,
            bin_step: tracked.bin_step || poolMeta?.bin_step || null,
            volatility: tracked.volatility ?? null,
            fee_tvl_ratio: tracked.fee_tvl_ratio ?? null,
            organic_score: tracked.organic_score ?? null,
            amount_sol: tracked.amount_sol || null,
          ...
          log("close_warn", `External-close backfill failed for ${position_address.slice(0, 12)}: ${e.message}`);
        }
      }
