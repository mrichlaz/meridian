...
// ── Patch flat records from on-chain truth ────────────────────────
let patched = 0;
let pnlDelta = 0;
let feesDelta = 0;
const patchedPositions = new Map();
for (const rec of perf) {
  if (!rec.position || !isFlat(rec)) continue;
  ...
  const before = { pnl_usd: rec.pnl_usd, fees: rec.fees_earned_usd };
  ...
  patched++;
  console.log(`  patch ${rec.pool_name || truth.pair} ${rec.position.slice(0, 8)}: 0.00%/$0.00 → ${rec.pnl_pct}%/$${rec.pnl_usd} (fees $${rec.fees_earned_usd})`);
}
...
if (fs.existsSync(poolMemoryPath) && patchedPositions.size > 0) {
  ...
  for (const entry of Object.values(poolMemory)) {
    for (const dep of entry?.deploys || []) {
      ...
      if (!rec || (Number(dep.pnl_usd) || 0) !== 0) continue;
...
// ── Report / write ───────────────────────────────────────────────
console.log("");
console.log(`Flat records patched:      ${patched} (PnL ${pnlDelta >= 0 ? "+" : ""}$${pnlDelta.toFixed(2)}, fees +$${feesDelta.toFixed(2)})`);
