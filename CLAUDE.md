...
| `tools/chart-indicators.js` | ~330 | `confirmIndicatorPreset({mint, side})`. Nine presets: `no_downtrend` (veto-only gate built for the single-sided-below LP book — blocks confirmed downtrends, allows chop/uptrend), `supertrend_break`, `rsi_reversal`, `bollinger_reversion`, `rsi_plus_supertrend`, `supertrend_or_rsi`, `bb_plus_rsi`, `fibo_reclaim`, `fibo_reject`. Fetches from Agent Meridian `/chart-indicators/{mint}`. Fail-open: API errors confirm. |
...
| `screening` | `excludeHighSupplyConcentration`, `minFeeActiveTvlRatio`, `minTvl`, `maxTvl`, `minVolume`, `minOrganic`, `minQuoteOrganic`, `minHolders`, `minMcap`, `maxMcap`, `minBinStep`, `maxBinStep`, `timeframe`, `category`, `minTokenFeesSol`, `useDiscordSignals`, `discordSignalMode`, `avoidPvpSymbols`, `blockPvpSymbols`, `maxBotHoldersPct`, `maxTop10Pct`, `allowedLaunchpads`, `blockedLaunchpads`, `minTokenAgeHours`, `maxTokenAgeHours`, `athFilterPct`, `maxEntryPriceRunup1hPct`, `maxEntryPriceDrop1hPct` | see `user-config.example.json` |
...
- **Rule 8**: in-range single-sided ladder with ≥ `conversionExitPct` (default 65, 0=off) of bins converted to base token AND effective loss PnL ≤ -2% → close. Catches deep ranges that stay "in range" through a big bleed so no OOR rule ever arms.
- **Stop-loss V-bottom guard**: a shallow stop breach must HOLD for `stopLossConfirmMinutes` (default 3, 0=off) before closing — `pending_stop_loss_since` is stamped by `updatePnlAndCheckExits` on first breach, recovery above the stop cancels it (a null tick does NOT cancel), and Rule 1 in `getDeterministicCloseRule` defers while the window is open. Deep breaches (>4pp past the stop) always close immediately — rugs don't bounce. Evidence (Jul 12-14): 3 of 4 febu stop-outs sold within an hour of a local V-bottom (+14-16% 1h after the stop), while real dumps kept falling.
- **Loss-side exits use `effectiveLossPnlPct` (state.js)**: non-suspicious ticks act on `pnl_pct_derived` (fresh on-chain mark), NOT `pnl_pct` — which prefers the provider's precomputed pct (display parity with the DLMM UI) and can lag minutes behind on a fast bleed. Suspicious ticks require both sources to agree (max of the two).
- **Rule 2 (take profit) uses `effectiveProfitPnlPct` (min of reported/derived when both exist) AND requires the confirmed `peak_pnl_pct` ≥ TP** — the peak only updates after the 15s two-phase recheck, so one phantom provider tick can't fire it (Jul 12: a glitched reported pct with derived=null closed a -1.33% position as "take profit"; the sanity flag can't raise when derived is null since pnl_pct_diff is null). A genuine run-up confirms its peak in one recheck and fires on the next evaluation.
- **Peak recheck ratchets on spike-and-fade**: when the 15s recheck reads below the 0.85× tolerance, `resolvePendingPeak` no longer discards the candidate outright — it ratchets `peak_pnl_pct` to `min(candidate, recheck)` if that beats the current confirmed peak (both real ticks printed at/above that level, so it's corroborated). Phantom ticks still can't pass: their recheck reads the true PnL, which won't exceed the existing peak. (Jul 12: a real +3.54% spike faded to +2.22% inside the 15s window; the all-or-nothing reject kept peak ≈0, trailing TP never armed, and the position round-tripped the whole move.) Note: trailing still only arms when the *corroborated* peak reaches `trailingTriggerPct` — a spike whose corroborated floor is below the trigger won't arm it.
- **External closes (Meteora web / Phantom) are backfilled**: `syncOpenPositions` returns the positions it auto-closed and `getMyPositions` runs `backfillExternalClose` (dlmm.js) on each — recordPerformance from the `last_live_*` snapshot with reason "external close via wallet", so pool-memory cooldowns and the per-token 24h cap count manual closes too. `recordPerformance` dedups by position address (one record per position, ever). Deterministic rule closes also pass `reason` into `close_position` now (previously defaulted to "agent decision", which skipped the low-yield annotation and OOR cooldown counting). **The auto-close grace is anchored to `missing_since` (first tick the position vanished), not deploy time** — an agent-initiated close makes the position vanish on-chain seconds before `closePosition` finishes its bookkeeping, and the deploy-anchored grace let the PnL poller instantly backfill a fabricated 0.00% "external close" record that dedup-blocked the real numbers (Jul 12: every close for hours recorded as flat/unpriced). Reappearance clears `missing_since`, so a transient RPC gap can't mass-close positions. Both `getMyPositions` paths (RPC `computePositions` and Meteora-API) persist `last_live_*` snapshots each tick, and `backfillExternalClose` refuses to record when no live economics exist (it would otherwise fabricate `final = initial`).
- **`initial_value_usd` is executor-derived, never trusted from the caller**: the deploy_position safety check overwrites it with `amount_y × live SOL price` (executor.js). The tool schema still exposes the field, but the SCREENER LLM fills it from its own idea of the SOL price (Jul 13: ~2.3× the live price on six deploys), and any later PnL computed against the inflated basis — external-close backfill, close paths with no live mark — records a phantom ~-57% loss on a close that was actually profitable AND derives a poisoned FAILED lesson from it. `scripts/backfill-from-export.mjs --fix-mismatch` repairs such records from a wallet export; lessons derived from them must be pruned by hand (they carry `pnl_pct`/`close_reason` from the bad record).
- **Markout (adverse-selection meter)**: `recordPoolDeploy` computes each deploy's net PnL at +15m/+60m from the position's own pool-memory snapshots (`computeDeployMarkouts`); per-pool `avg_markout_15m/60m` is surfaced in `recallForPool` with a TOXIC FLOW flag below -2% at 60m.
- **Per-token deploy cap**: `getBaseMintDeployCap` (pool-memory.js) counts CLOSED deploys across all pools sharing a base mint in the last rolling 24h against `maxDeploysPerToken24h` (default 5, 0=off). Checked in `deployPosition` (after the mint-cooldown check) and as a `getTopCandidates` hard filter. Rationale: deploy #6+ into the same token averaged -$0.14 (n=107, Jul 1-7) vs +$1.13..+$3.49 for #1-4; the repeat-deploy cooldown misses these because one zero-fee churn close resets its consecutive-fee-earning streak.
- **Quiet hours (UTC size damping)**: `getQuietHourAdjustment` (policy-engine.js) parses `policyQuietHoursUtc` ("8-12,20-24", half-open, wraps midnight) and multiplies `sizeMultiplierForScore` output by `policyQuietHoursSizeMult` (default 0.5, clamped [0.1,1] — 0 would fail deployPosition's positive-amount validation). Applies only to the automatic screening paths; manual `/deploy` is unaffected. **Self-learning**: `evolveQuietHours` (lessons.js, gated by `policyQuietHoursAuto`, default true) re-derives the windows every evolve tick from the last 200 closes — a 4h UTC block goes quiet when it has ≥12 closes AND net-negative PnL, max 2 blocks, deploy hour taken from `deployed_at` or `recorded_at − minutes_held`; an all-positive history clears the damping. Manual `policyQuietHoursUtc` edits get overwritten unless `policyQuietHoursAuto=false`.
- **ML λ gate is the walk-forward RANK lift (`cvLift`)** — top-half vs bottom-half win rate of model-scored validation trades, per chronological fold — not classification accuracy (`cvEdge`, kept as diagnostic; the ~80% majority base rate from the 1% label hurdle makes accuracy structurally unbeatable). Training uses a recency window (`mlTrainWindow`, default 500 closes).
- **Dead-man watchdog**: the daemon writes `data/heartbeat.json` after every management cycle (+ startup); `scripts/watchdog.mjs` (run from SYSTEM cron, e.g. every 10 min) alerts Telegram when the heartbeat is >20 min stale with open positions, rate-limited to 1/hour via `data/watchdog-state.json`.
- **Exit-rule replay harness**: `node scripts/replay-exits.mjs [--export <wallet-export.json>] [--since YYYY-MM-DD]` evaluates time-stop and stop-loss grids against closed-position history (endpoint estimates labeled optimistic/conservative; exact where pool-memory snapshot paths exist). Run it before changing exit thresholds.
